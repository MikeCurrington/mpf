...
class CanPinSerialCommunicator(BaseSerialCommunicator):

    """Manages a Serial connection to the first processor in a CanPin serial chain."""
    ...
    def __init__(self, platform: "CanPinHardwarePlatform", port, baud) -> None:
        """Initialise Serial Connection to CanPin Hardware."""
        self.part_msg = b""
        self.chain_serial = None    # type: str
        self._lost_synch = False
        self.read_task = None       # type: (method)
    
        print("CanPinSerialCommunicator port: " + str(port) + " baud: " + str(baud))
